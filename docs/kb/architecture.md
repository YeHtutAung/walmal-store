...
**All three catalog fetchers (`fetchProducts`, `fetchProductsByCategory`, `fetchProductsSSG` in `src/lib/api/products.ts`) pass `status=ACTIVE`** — walmal's opt-in status param (2026-07-18; see `../walmal/docs/kb/SYSTEM.md`). Deactivated products (including admin-E2E residue) can never appear on the storefront; the admin intentionally omits the param because its list needs INACTIVE rows.
...
| `(checkout)/cart/` | `/cart` — "Your Bag" page, see below |
...
**Bag page (`/cart`, `src/app/(checkout)/cart/page.tsx`):** rebuilt as a two-column layout — `BagLineItem` rows (`src/components/cart/bag-line-item.tsx`) + `BagSummary` (`src/components/cart/bag-summary.tsx`), `lg:grid-cols-[1fr_384px] gap-10`. Reads `useCart()` (unchanged hook) and gates the whole items-vs-empty decision behind `useMounted()` — same hydration-guard rule as `/saved` (persisted `walmal-cart` store; server HTML can't know the cart contents). `BagLineItem`'s qty stepper calls `updateQty(variantId, quantity ± 1)` directly; the **− button is disabled in the UI at quantity 1** — it never dispatches `updateQty(id, 0)` (the store's remove-at-`qty<=0` semantics exist for the drawer's own stepper, not this page). `BagSummary` derives `freeDeliveryProgress(subtotal)` from `src/lib/free-delivery.ts` (`FREE_DELIVERY_THRESHOLD = 80`, deliberately coupled in a code comment to the announcement bar's "Free local delivery over $80" copy — change both together): under threshold renders a red progress bar + "Add {formatPrice(remaining)} more for free local delivery." caption; at/over threshold renders a green `Delivery — Free` row instead. **Total always renders the Subtotal, unconditionally** — no tax/fee line is ever shown (spec decision: never display a charge Stripe does not actually collect at checkout); the "Shipping and taxes calculated at checkout." disclaimer keeps the same copy the drawer already used. The cart **drawer** (`cart-drawer.tsx`/`cart-item.tsx`/`cart-summary.tsx`) is untouched by this rebuild — it keeps its own simpler markup and remains what the header/tab-bar Bag button opens; the `/cart` page is a separate, richer destination for direct navigation.
...
- `GET /api/preview` / `GET /api/preview/disable` — Home-Page CMS **Draft Mode** preview. `/api/preview?token=…` compares `token` to server-side `CONTENT_PREVIEW_TOKEN` (401 on unset/mismatch), enables `draftMode()`, redirects to `/`; `/disable` exits. The admin's "Preview" button opens `/api/preview?token=…`. See Homepage below.
- The legacy `src/app/api/v1/*` mock routes and `src/lib/mock-db.ts` were **deleted 2026-07-18** (Phase 1 hygiene) — every catalog/order/cart call goes to the real Spring backend.
...
`SiteHeader` (client) renders, in order: `DemoBanner` (test-mode notice — "nothing is charged", 4242 test card; dismissal persists at localStorage `walmal-demo-banner-dismissed`, mounted-gated; the E2E `clearState` helper pre-seeds the dismissal so only TC-E2E-042 sees it), then `AnnouncementBar` (server component; red `bg-primary` strip, `label-caps`, "Free local delivery over $80 · Worldwide shipping available") as a non-sticky sibling above the sticky `<header>` (`bg-background/90 backdrop-blur border-b border-border`, inner `max-w-[1360px]`), then `CartDrawer` (standalone mode only, see above) and `MobileMenu` as siblings below.
...
- **Home-Page CMS (admin-editable, added 2026-07-21):** `hero.tsx`, `category-tiles.tsx`, `promo-banner.tsx` each take an OPTIONAL content prop and render admin-managed content from the backend when present, else fall back to the EXACT static JSX below. `src/lib/api/home-content.ts` `fetchHomeContent()` reads `GET /api/v1/content/home` (published, ISR `revalidate:3600`) — or `GET /content/home/draft?previewToken=…` with `cache:'no-store'` when `draftMode()` is enabled — and returns `null` on 204/error (→ static fallback, so the pre-first-publish home is byte-for-byte unchanged). `page.tsx` passes `home?.hero`/`?.categoryTiles`/`?.promo`. Dynamic image URLs go through `resolveMinioUrl` (the `content-images` bucket rides the existing `/api/minio` proxy); headline/heading `\n` → `<br/>`. Server-side env `CONTENT_PREVIEW_TOKEN` (NOT `NEXT_PUBLIC`; must equal the backend's). Spec: `../walmal/docs/superpowers/specs/2026-07-21-editable-home-page-cms-design.md`.
- `hero.tsx`, `promo-banner.tsx`, `trust-bar.tsx`, `category-tiles.tsx` — server components; static-fallback art from `/public/sport/*.svg` (plain `<img>`, CSP-safe, gradient overlays per wireframe). **`category-tiles.tsx`'s static fallback hardcodes the four category slugs (`jerseys`/`boots`/`teamwear`/`equipment`) — coupled to the V17 seeded taxonomy** (`../walmal` V17 migration); desktop 4-up image tiles, mobile horizontal chip rail (with an "All" chip → unfiltered `/products`). Both renderings are driven by CMS `tiles` when published.
